...
if (phy_position_x > 5500) {
	instance_destroy();	
}

if (abs(x - obj_player.x) < 220) {
	if (alpha < 1) {
		alpha += 0.05;	
	}
} else {
...
if (alpha > 0) {
	...
	draw_sprite_ext(spr_key_icon,image_index,x+80,y-15,0.35 * scl,0.35 * scl,key_angle,c_white,alpha);
	draw_set_color(c_black);
	draw_set_valign(fa_center);
	draw_set_halign(fa_middle);
	draw_set_font(ft_start);
	draw_text_transformed(x+80,y-15,"E",scl,scl,key_angle);
